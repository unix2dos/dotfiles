...
# Copies the resolved Preset into a self-contained plugin directory
# (skills/ + plugin manifests). Real files, not symlinks — commit it
# to git so cloud Agents can install this repo as a plugin.
...
EOF
  ...
  log_info "commit + push the bundle dir to update cloud installs"
}
