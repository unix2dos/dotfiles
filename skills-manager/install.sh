...
# ============================================
# 读取 config.yaml，完成：
#   clone → build → aggregate → 按目录分发 → 打包 bundle → 安装原生扩展
#
# 用法:
...
step_clean() {
  log_header "Step 0/7 — clean stale dirs"
...
step_clone_and_build() {
  log_header "Step 1/7 — clone + build"
  ...
  while IFS= read -r source_name; do
    ...
    if [[ -n "$single_skill" ]]; then
      ...
      if [[ -f "$local_dir/SKILL.md" ]]; then
        sed -i.bak "s/^name: .*/name: ${source_name}/" "$local_dir/SKILL.md"
...
step_aggregate() {
  [[ "$PREVIEW" == true ]] || log_header "Step 2/7 — aggregate -> ${INSTALL_DIR}"
...
step_link() {
  log_header "Step 3/7 — distribute to Destinations"
...
# ─── Step 4: bundle for cloud Agents ──────────────────────────────
# Copies the resolved Preset into a self-contained plugin repository
# (skills/ + plugin manifests). Real files, not symlinks — the target
# is a dedicated git repo (bundle.path); with bundle.push the script
# commits and pushes it so cloud installs stay in sync.
...
resolve_bundle_dir() {
  ...
  case "$raw_path" in
    ...
    *)  echo "$SCRIPT_DIR/$raw_path" ;;
  esac
}

step_bundle() {
  local raw_path preset bundle_dir bundle_name
  raw_path=$(q '.bundle.path')
  [[ -z "$raw_path" ]] && return 0
  preset=$(q '.bundle.preset')
  [[ -n "$preset" ]] || { log_error "bundle.path set but bundle.preset missing"; return 1; }
  yq -e ".presets[\"${preset}\"]" "$CONFIG" >/dev/null 2>&1 || {
    log_error "bundle references unknown Preset: ${preset}"
    return 1
  }
  bundle_name=$(q '.bundle.name')
  bundle_name="${bundle_name:-skills-bundle}"
  bundle_dir=$(resolve_bundle_dir "$raw_path")

  log_header "Step 4/7 — bundle -> ${bundle_dir}"

  local -a skills=()
  local s
  while IFS= read -r s; do
    [[ -n "$s" ]] && skills+=("$s")
  done < <(resolve_preset_skills "$preset")

  if [[ "$DRY_RUN" == true ]]; then
    log_info "[DRY-RUN] would write ${#skills[@]} Skills + plugin manifests under ${bundle_dir}"
    [[ "$(q '.bundle.push')" == "true" ]] && log_info "[DRY-RUN] would commit + push ${bundle_dir} if dirty"
    return 0
  fi

  # Bundle dir is generated output; only the trees this script owns are replaced.
  local skills_dir="$bundle_dir/skills"
  rm -rf "$skills_dir"
  mkdir -p "$skills_dir" "$bundle_dir/.devin-plugin" "$bundle_dir/.claude-plugin"

  local copied=0 missing=0 skill target
  for skill in ${skills[@]+"${skills[@]}"}; do
    target=$(map_value "$SKILL_DIR_MAP" "$skill" 2>/dev/null || true)
    if [[ -n "$target" && -f "$target/SKILL.md" ]]; then
      rsync -aL --exclude='.git' "$target/" "$skills_dir/$skill/"
      ((copied++)) || true
    else
      log_warn "bundle: Skill not found: ${skill}"
      ((missing++)) || true
    fi
  done

  # No 'version': Claude Code resolves plugin.json version → marketplace
  # version → git SHA; omitting it makes every commit a new version.
  local manifest owner
  manifest=$(cat <<EOF
{
  "name": "${bundle_name}",
  "description": "Aggregated Agent skills generated by skills-manager; do not edit."
}
EOF
)
  printf '%s\n' "$manifest" > "$bundle_dir/.devin-plugin/plugin.json"
  printf '%s\n' "$manifest" > "$bundle_dir/.claude-plugin/plugin.json"

  owner=$(q '.bundle.owner')
  owner="${owner:-$(git config --get github.user 2>/dev/null || echo unknown)}"
  cat > "$bundle_dir/.claude-plugin/marketplace.json" <<EOF
{
  "name": "${owner}",
  "owner": { "name": "${owner}" },
  "plugins": [
...
EOF
  ...
  if [[ "$missing" -gt 0 ]]; then
    log_warn "bundle: ${copied} copied, ${missing} missing"
  else
    log_success "bundle: ${copied} Skills -> ${bundle_dir}"
  fi

  local do_push
  do_push=$(q '.bundle.push')
  if [[ "$do_push" == "true" ]]; then
    if [[ ! -d "$bundle_dir/.git" ]]; then
      log_warn "bundle.push=true but ${bundle_dir} is not a git repo; skipped push"
    elif [[ -z "$(git -C "$bundle_dir" status --porcelain)" ]]; then
      log_info "bundle: no changes, skip push"
    else
      git -C "$bundle_dir" add -A
      if git -C "$bundle_dir" commit --quiet -m "chore: sync skills bundle" && \
         git -C "$bundle_dir" push --quiet; then
        log_success "bundle pushed to remote"
      else
        log_warn "bundle commit/push failed; sync manually later"
      fi
    fi
  else
    log_info "commit + push the bundle repo to update cloud installs"
  fi
}
...
step_preview() {
  ...
  local bundle_path bundle_preset bundle_skills
  bundle_path=$(q '.bundle.path')
  if [[ -n "$bundle_path" ]]; then
    bundle_preset=$(q '.bundle.preset')
    bundle_skills=$(resolve_preset_skills "$bundle_preset")
    echo ""
    echo "BUNDLE"
    echo "  path: $(resolve_bundle_dir "$bundle_path")"
    echo "  preset: $bundle_preset"
    echo "  skills: $(printf '%s\n' "$bundle_skills" | count_lines)"
    if [[ "$PREVIEW_FULL" == true ]]; then
      printf '%s\n' "$bundle_skills" | while IFS= read -r skill; do [[ -n "$skill" ]] && echo "    - $skill"; done
    fi
  fi
...
step_extensions() {
  ...
  [[ "$action" == "preview" ]] || log_header "Step 6/7 — native Extensions"
